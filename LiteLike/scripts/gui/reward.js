...
export class RewardsGui{
     ...
     */
    loadRewardEvent(){
        let rewards = this.encounteroptions.rewards;
    ...
    addReward(reward){
        ...
        let strings = IO.getStrings(this.game.STRINGS, reward.reward);
    ...
    <div class="splitbutton">
        ...
        <div class="dropdownarrow">
            ...
            row.querySelector("button.collection").onclick = (event)=>this.collectReward(event, row, reward, player);
            // Hovering over the dropdown arrow populates a list of items the player can drop to make space
            row.querySelector("div.dropdownarrow>button").addEventListener("mouseover", (event)=>this.updateCache(event, row, reward, player));
            ...
            row.querySelector("div.dropdownarrow>div.dropdownlist").addEventListener("click", (event)=>this.exchangeCache(event, row, reward, player));
    }
    ...
    collectReward(event, row, reward, player){
        if(!player || typeof player == "undefined") player = this.game.PLAYER;
        // Getting key stats
        ...
        if(collectquantity < qty){
            ...
            this.encounteroptions.rewards.splice(this.encounteroptions.rewards.indexOf(reward), 1);
        }
     ...
     */
    updateCache(event, row, reward, player){
        if(!player || typeof player == "undefined") player = this.game.PLAYER;
        // Getting key stats
        let capacity = player.equipment.transport.capacity;
        let carryweight = player.weight;
        ...
        for(let item of [...player.weapons, ...player.items, ...Object.values(player.resources)]){
            ...
            strings = IO.getStrings(this.game.STRINGS, item);
     ...
     * @param {Character} player - The player character
     */
    exchangeCache(event, row, reward, player){
        if(!player || typeof player == "undefined") player = this.game.PLAYER;
        ...
        if(type == "Weapon"){
            ...
            this.collectReward(event, row, reward, player);

            // Add Dropped Item back into reward area
            this.addRewardToEvent(new ENCOUNTERS.Reward("Weapon", item));
        }
        else if(type == "Item"){
        ...
        // Add Dropped Item back into reward area
        this.addRewardToEvent(new ENCOUNTERS.Reward(type, newreward));
    }
